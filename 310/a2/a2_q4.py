...
#             for k in range(105):
#                 color += [k]
#                 csp = MapColoringCSP(color, graphs[j])
...
#             print("The count of the number of times CSP variables were conflictes: ", num_conflicte)
#
#             print()
def min_conflicts_result(graph):
    colors = []
    for j in range (100):
        colors = colors+[j]
        csp = MapColoringCSP(colors, graph)
        result = min_conflicts(csp, max_steps = 1000)
        num_assign = csp.nassigns
        num_unassign = csp.nunassigns
        if result != None: return result, num_conflicte,num_assign,num_unassign
...
def run_q4():
    ...
    for i in range(5):
        ...
        for j in range(6):
            ...
            result, num_conflicte,num_assign,num_unassign = min_conflicts_result(graphs[j])
            num_teams = max(result.values()) + 1
            end_time = time.time()
